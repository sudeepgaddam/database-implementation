...
char *binfilespath="";
char *tblfilespath="/cise/tmp/dbi_sp11/DATA/10M/";
...
void Catalog::initializeCatalogDS()
{
     /*Logic:
...
void Compiler::RunDDLquery()
{
    ...
    {
        Catalog *c = myCatalog->getInstance();
...
void MyQueryRunner::performoperation(QueryPlanNode *node)
{
 ...
 /*           treeroot->cnf->leftrightJoinAtts(&x,&y);
            ...
            jOp[jcount]->Run(*pipes[treeroot->inputPipe1],*pipes[treeroot->inputPipe2],*pipes[treeroot->outPipe],*treeroot->cnf,*treeroot->lit);
            jcount++;
        }
